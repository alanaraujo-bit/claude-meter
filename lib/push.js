...
export async function maybeAlert(summary, insights) {
  ...
  for (const p of insights.per) {
    ...
    // Sem horário de reset não há como identificar a janela, e sem isso o
    // controle de repetição não funciona — melhor não notificar.
    if (!p.resetsAt) continue;

    // A janela é identificada pelo seu reset (absoluto e estável), não pelo
    // início estimado. Virou a janela ou subiu de nível, avisa de novo.
    const ins = await query(
      `INSERT INTO alert_state (account, window_start, level)
       VALUES ($1, $2, $3)
       ON CONFLICT (account, window_start, level) DO NOTHING`,
      [p.account, p.resetsAt, p.level]
    );
